...
def non_async_make_snapshot(model, optimizer, use_copy_, use_timer, step_cnt, timer_record_file, non_blocking_copy):
                ...
                with record_function(f"model_training_{step_cnt}"):
                    ...
                    if args.use_snapshot:
                        ...
                        if args.async_snapshot:
                            checkpoint_thread = async_ckpt.make_snapshot(model, optimizer, epoch, use_timer=args.use_timer, step_cnt=step_cnt, timer_record_file=snapshot_timer_record_file, use_copy_=args.use_copy)
                            async_checkpoint_thread_list.append(checkpoint_thread)
                        else:
                            non_async_make_snapshot(model, optimizer, args.use_copy, args.use_timer, step_cnt, snapshot_timer_record_file, args.non_blocking_copy)
