...
from async_snapshot.async_snapshot import AsyncShardedCheckpoint
...
def pretrain(train_valid_test_dataset_provider,
    ...
    # print("initialize_megatron ninja available", cpp_extension.is_ninja_available())
...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    nmodel = model[0]
    # with open(f"/data2/share/md_test/reft_plus/info/tp_{mpu.get_tensor_model_parallel_rank()}_dp_{mpu.get_data_parallel_rank()}_pp_{mpu.get_pipeline_model_parallel_rank()}.txt", "w") as f:
    #     for param_tensor in nmodel.state_dict():
    #         # Print the tensor name and its size
    #         # print(param_tensor, "\t", nmodel.state_dict()[param_tensor].size())
    #         f.write(f"{param_tensor} \t {nmodel.state_dict()[param_tensor].size()}\n")
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    args = get_args()
    timers = get_timers()
    snapshot_stream = torch.cuda.Stream(device=torch.cuda.current_device())
    ...
    def trace_handler(prof):
        ...
        trace_file_path = os.path.join("/data2/share/md_test/reft_plus/trace", f"{time_stamp}_rank_{torch.distributed.get_rank()}.json")    
