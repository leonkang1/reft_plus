...
class AsyncCheckpoint:
    ...
    def make_snapshot(self, model, optimizer, epoch, use_timer, step_cnt, timer_record_file, use_copy_, snapshot_stream, device, train_stream):
        # with self.thread_lock:
        #     asyncio.run(self.allreduce_semaphore.acquire())

        checkpoint_thread = threading.Thread(
            target=self._snapshot_thread,
            args=(model, optimizer, epoch, use_copy_, use_timer, step_cnt, timer_record_file, snapshot_stream, device, train_stream)
        )
        checkpoint_thread.start()
        return checkpoint_thread
        
    def _snapshot_thread(self, model, optimizer, epoch, use_copy_, use_timer, step_cnt, timer_record_file, snapshot_stream, device, train_stream):
        if use_timer and step_cnt > 10:
            start_time = time.perf_counter()
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            loop.run_until_complete(self._make_snapshot(model, optimizer, epoch, use_copy_, snapshot_stream, device, train_stream))
        finally:
            loop.close()
        if use_timer and step_cnt > 10:
    ...
    async def _make_snapshot(self, model, optimizer, epoch, use_copy_, snapshot_stream, device, train_stream):
        # snapshot_stream.wait_stream(train_stream)
