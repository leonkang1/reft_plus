...
def train(args):
    ...
    # end_of_step_event = torch.cuda.Event()
                ...
                with record_function(f"model_training_{step_cnt}"):
                    ...
                    if args.use_snapshot:
                        ...
                        if args.async_snapshot:
                            checkpoint_thread = async_ckpt.make_snapshot(model, optimizer, epoch, use_timer=args.use_timer, step_cnt=step_cnt, timer_record_file=snapshot_timer_record_file, use_copy_=args.use_copy, snapshot_stream=snapshot_stream, device=device)
                    ...
                    # end_of_step_event.wait(stream=torch.cuda.default_stream(device))
